...
function IREmp(req, res){
  const {Cod_Emp,Horas_Extras,Dias_P
    ,Dias_A,Dias_F,Dias_Totales,Mes_vigente,Anhos_Antiguedad} = req.body;
  console.log(Cod_Emp,Horas_Extras,Dias_P
    ,Dias_A,Dias_F,Dias_Totales,Mes_vigente,Anhos_Antiguedad);
  mysqlConnection.query('insert into registros_emp(Cod_Emp,Horas_Extras,Dias_P,Dias_A,Dias_F,Dias_Totales,Mes_vigente,Anhos_Antiguedad)values(?,?,?,?,?,?,?,?)' ,
        [Cod_Emp,Horas_Extras,Dias_P
            ,Dias_A,Dias_F,Dias_Totales,Mes_vigente,Anhos_Antiguedad], (err, rows, fields) => {
       if(!err) {
...
function AREmp(req, res){
  ...
  mysqlConnection.query('update registros_emp set Cod_Emp = ?, Horas_Extras = ?,Dias_P = ?,Dias_A = ?,Dias_F = ?,Dias_Totales = ?, Mes_vigente = ?, Anhos_Antiguedad = ?  where cod_Registros_Emp = ?',
       [Cod_Emp,Horas_Extras,Dias_P
